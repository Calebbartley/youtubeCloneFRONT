...
const Comment = ({comment, deleteComment, putComment, index, postReply}) => {
    ...
    return ( 
        <li key={index}> {comment.userName} {comment.userComment}  <button  onClick={() => deleteComment(comment._id)}> Delete💢</button> <button onClick={() => putComment(comment._id)} >Edit Comment</button>
